...
bool PlayerWentAFK[MAXPLAYERS+1];
Handle hSpec;
Handle hSwitch;
...
public void OnPluginStart()
{
	...
	CreateConVar("l4dafkfix_version", PLUGIN_VERSION, "Version of L4D 4+ Survivor AFK Fix", FCVAR_SPONLY|FCVAR_REPLICATED|FCVAR_NOTIFY|FCVAR_DONTRECORD);
	FindConVar("l4dafkfix_version").SetString(PLUGIN_VERSION);

	// Read file handle
	GameData temp = new GameData("l4dafkfix");
	if (temp != null)
	{
		// USED SetHumanIdle
		...
		if (hSpec == null) SetFailState("Survivor Afk Fix: SetHumanSpec Signature broken");
		...
		if (hSwitch == null) SetFailState("Survivor Afk Fix: TakeOverBot Signature broken");
		delete temp;
	}
	else
	{
...
public Action TakeOverMe(int client, int args)
{
...
public Action GoAwayFromKeyboard(int client, int args)
{
	...
	if (GetClientTeam(client) == 1)
		CPrintToChatAll("{blue}%N\x01 has become a {blue}spectator.", client);
...
public Action Timer_ActivateFix(Handle Timer, DataPack datapack)
{
	// Reset the data pack
	datapack.Reset();
